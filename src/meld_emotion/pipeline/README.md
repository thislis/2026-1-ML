...
비교표(콘솔 + JSON)를 낸다. 교수님 피드백의 Early/Late fusion 비교, SVM/LogReg 베이스라인
비교 등을 한 명령으로 수행하기 위한 것이다.
...
**경계 내성**: 어떤 변형이 미구현 경계(`@unimplemented`, 예: MELD I/O·SVM)에 닿아 예외를
...
- 학습/평가 분할과 학습 시 modality dropout 은 설정으로 제어한다(`ExperimentConfig` 의
  `train_split`/`eval_split`/`dropout`). 빌더가 이를 러너에 연결하므로 YAML 만으로 켤 수 있다:
