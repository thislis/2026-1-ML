...
- `feature_pipeline.py` — `FeaturePipeline`: 추출기들을 학습 분할로 `fit` 후 임의 분할을
  `FeatureBundle` 로 변환하고 모달리티 가용성 마스크를 구성한다. 비디오 추출기가 있고
  `VideoInput.source_path` 만 있는 샘플은 특징 추출 전 `MediaLoader` 로 프레임을 lazy-load 한다.
...
비디오 특징 추출기가 포함된 MELD 실험에서는 흐름 앞단에서 MP4 오디오를 추출하지 않고 비디오
프레임만 적재한다. 기본은 32프레임 균등 샘플링 + 64×64 resize 이며 `ExperimentConfig.media`
또는 YAML 의 `media.video_max_frames`/`media.video_frame_size` 로 바꾼다.
`MediaLoader` 는 오디오 디코딩 미구현 때문에 status 에서는 UNIMPLEMENTED 로 남아 있지만,
`load_video` 경로는 이 흐름에서 실제 사용된다.
...
**경계 내성**: 어떤 변형이 미구현 경계(`@unimplemented`, 예: raw 오디오 디코딩)에 닿아 예외를
...
- 학습/평가 분할, 학습 시 modality dropout, raw MP4 비디오 적재 옵션은 설정으로 제어한다
