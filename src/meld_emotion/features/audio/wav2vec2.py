...
발화 단위 dense embedding 으로 변환한다. 학습 상태가 필요 없어 ``fit`` 은 no-op 이며, 출력은
예측용 ``FeatureKind.EMBEDDING`` 행렬이다.
"""
...
def _load_transformers_classes() -> tuple[Any, Any]:
    ...
    try:
        processor_cls = module.__dict__["AutoProcessor"]
        model_cls = module.__dict__["Wav2Vec2Model"]
    except KeyError as exc:
        raise ImportError("The 'transformers' package does not expose Wav2Vec2 classes.") from exc
...
class Wav2Vec2XlsrAudioExtractor(BaseFeatureExtractor):
    ...
    def transform(self, samples: Sequence[RawSample]) -> FeatureMatrix:
        ...
        for start in range(0, len(valid), self._batch_size):
            ...
            embeddings = self._embed_batch([wave for _, wave in batch])
